...
def output_data(ip,db_name,port,shuihao,company_name):
    ...
    os.makedirs('/common/pythonCode/开票明细/{company_name}/开票明细'.format(company_name=company_name),exist_ok=True)
    #连接数据库
    ...
    for i in shuihao:
        ...
        WHERE A.serial_num=B.serial_num AND nsrsbh='{shuihao}' AND kprq BETWEEN '{s_y}-{s_m}-01 00:00:00'AND'{e_y}-{e_m}-31 23:59:59' ORDER BY kprq
        '''.format(shuihao=shuihao[name],s_y=year,s_m=month,e_y=year,e_m=month)
	
        f_name="/common/pythonCode/开票明细/{company_name}/开票明细/{filename}.xlsx".format(company_name=company_name,filename=name)
        #如果某个税号已经导出过了,不会再导出
